...
- **Status:** approved to build (user, 2026-09: "let's build phase 3"). **Implemented:** runner `tools/run_anomaly_engine_ee.py` SHA-256 `73bd021c76d46ae42180161580421321611efaf7d4246029ed047965fbf9a9f7` (`anomaly_engine_python_api_v1_lake_wide_candidate_c`); supervisor `tools/supervise_anomaly_engine.py` (`anomaly_engine_windows_supervisor_v1`, pinning that runner + the AUDIT-010 base supervisor). Both `--self-test` suites pass. `--build-climatology` was queued 2026-09; it was waiting on a transient 503 outage of the EEA datashare provenance endpoint (`sdi.eea.europa.eu`) — the geometry service and GML endpoint were up. Once one live verification succeeds it is cached hash-pinned (below) and later runs need no EEA access.
...
| `classification` | from `historical_percentile` (`METH-004`): `< 10th` below normal · `10–90th` within normal range · `90–95th` warm · `95–99th` unusually warm · `≥ 99th` extreme warm observation. If `percentile_confidence = insufficient_history` → `historical context unavailable` (anomaly shown, no label). If `confidence = none` → `no valid observation`. |
...
The approved geometry (`SPACE-001` / `AUDIT-003`) has a fixed, recorded identity: raw SHA-256 `5d5f9c8e…`, canonical SHA-256 `394a9296…`, 11,012 coordinate tuples. The anomaly engine verifies the geometry **once** (the full live `AUDIT-003` provenance chain via the pinned `verify_wise_huaih049.py`), then caches `(geometry_json, source_record)` locally under `local_run_state/phase3/` (git-ignored). Later runs load the cache only if its recorded identity matches the `AUDIT-003` pins exactly and the geometry still hashes to the cached value; otherwise they re-verify or fail closed. This removes a repeated dependency on the sometimes-unavailable EEA provenance servers from a production path, without weakening the provenance guarantee — the pinned canonical SHA-256 *is* the approved identity.
