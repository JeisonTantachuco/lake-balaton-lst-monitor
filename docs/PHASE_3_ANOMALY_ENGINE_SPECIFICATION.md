...
- **Boundary:** produces the climatology baseline, the daily anomaly records, the monthly summaries, and — under approved `ARCH-001` — the four Earth Engine assets the app reads. It does not build the interface (Phase 4) or the validation (Phase 5 / `VAL-001`). Local Phase 3 output stays coordinate-free; the one asset with geometry (`lake_boundary`) is the approved `ARCH-001` supersession, for the deployed app only.
